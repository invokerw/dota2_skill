...
ScriptedAbility::ScriptedAbility(Unit& caster, const AbilityDef& def, LuaState& lua)
    ...
    self_["target_unit"]  = [this](sol::object) -> Unit* {
        if (last_target_unit_id_ == kInvalidEntityId) return nullptr;
        World* w = this->caster().world();
        return w ? w->find(last_target_unit_id_) : nullptr;
    };
}
...
void ScriptedAbility::on_spell_start(CastContext& ctx) {
    ...
    last_target_unit_id_ = ctx.target.unit ? ctx.target.unit->id() : kInvalidEntityId;
