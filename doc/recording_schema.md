...
可选 `name` 字段: 投射物 particle / 美术资源名 (例如 `"particles/.../drow_frost_arrow.vpcf"`).
普攻 spawn 时由 attacker 上的 modifier (法球) 提供 -- 引擎挑首个非空字符串. 缺省 (空串) 表示走默认普攻投射物, 录像层用 unit 默认, 不影响命中逻辑.

```json
{"type": "projectile_spawn", "pid": 9, "src": 5, "origin": [0,0],
 "speed": 1250, "tracking": true, "target": 4,
 "name": "particles/units/heroes/hero_drow_ranger/drow_frost_arrow.vpcf"}
```
...
```json
{"type": "attack_landed", "src": 2, "dst": 3, "dmg": 48, "missed": false, "record": 17}
```

- `missed=true` 时 `dmg=0` (闪避).
- `record`: 关联的 `AttackRecord` id, 起始 `1`. 同一次普攻的 `projectile_spawn` (远程) 与 `attack_landed` 共享同一个 record id, 法球 modifier 在 OnAttack 认领的也是这个 record. 录像层据此把投射物 / 命中 / 落副作用 (减速 / 中毒 etc.) 关联起来.
